...
from dqn_models import Model, ConvModel
import argh
...
class ReplayBuffer:
    ...
    def insert(self, data):
        self.buffer[self.idx % self.buffer_size] = data
        self.idx += 1
...
def main( test=False, chkpt=None, device="cuda"):
    if not test:
        wandb.login(key="put your key here")
        wandb.init(project="dqn-breakout-v0", name="dqn_rmsprop")
    ...
    try:
        while True:
            ...
            rb.insert(Data(last_observation, action, reward, observation, done))
